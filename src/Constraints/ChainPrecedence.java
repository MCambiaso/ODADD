...
public class ChainPrecedence implements LCTemplateReplayer {
	...
	File file = new File("/home/matte/Scrivania/Out/OutChPrecedence.txt");
	FileWriter fw = null;
	BufferedWriter brf;
...
//			}		
		...
		if(previousChPrecedence!=null && !previousChPrecedence.equals("") && !previousChPrecedence.equals(event)){
			...
			if(nr>1){
				...
				mc = mod.addObservation(previousChPrecedence, event, myAttr, attribute, attIndex, 1, mc);
...
//					hoeffding tree
					...
					if(nr>1){
						...
						mc = mod.addObservation(first, event, myAttr, attribute, attIndex, 1, mc);
		...
		if(en==199999){ //instanceForTree.get(name).numInstances()>1000
			...
			for(String caseID : activityLabelsCounterChPrecedence.keySet()) {
				...
				if(Counter.containsKey("a-null")){
					...
					act = act + totnumber;
					if(fulfillForThisTrace.containsKey("b-null")){
						if(fulfillForThisTrace.get("b-null").containsKey("a-null")){	
							double currentFullfill = fulfillForThisTrace.get("b-null").get("a-null");
...
//				}
			...
			System.out.println("ChPrec"+"\t"+fulfill+"\t"+(act-fulfill));
//			printout.flush();
//			printout.close();
...
//		System.out.println("ChPr:\ttprocess:\t"+(System.currentTimeMillis()-start)+"\ttaddObs:\t"+time);
